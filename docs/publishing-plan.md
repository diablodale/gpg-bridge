...
_Import paths_ (all `.ts` files across both extensions, shared, and integration tests):
...
_Command ID strings_ in [commandExecutor.ts](../gpg-bridge-request/src/services/commandExecutor.ts),
...
_Runner path strings_ in [requestProxyRunTest.ts](../gpg-bridge-request/test/integration/requestProxyRunTest.ts)
...
_Configuration keys_ in both `extension.ts` files and integration tests:
...
_UI strings_ in [gpg-bridge-agent/src/extension.ts](../gpg-bridge-agent/src/extension.ts):
...
_UI strings_ in [gpg-bridge-request/src/extension.ts](../gpg-bridge-request/src/extension.ts):

- `createOutputChannel('GPG Request Proxy')` → `createOutputChannel('GPG Bridge Request')`

_Log strings_ across all `.ts` files in both extensions — apply the name mapping
...
> _before_ commit cannot inspect it. The ruleset rejection on push is the correct
> enforcement point.
...
Each file declares _when_ it runs (triggers) and _what it does_ (steps). Steps can
...
_Step 1 — Create the Azure DevOps PAT:_
...
_Step 2 — Store it as a GitHub repository secret:_
