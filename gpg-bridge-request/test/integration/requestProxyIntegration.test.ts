...
const { PublicKeySync } = require('../../services/publicKeySync') as typeof import('../../out/services/publicKeySync');
...
describe('Phase 6 — PublicKeySync with isolated gpg', function () {
    ...
    it('1. import round-trip: gpg imports the key; result has imported: 1', async function () {
        ...
        expect(targetFpr, 'imported key fingerprint must match source key fingerprint').to.equal(sourceFpr);
    });
    ...
    it('4. Import multiple groups of keys with duplicates', async function () {
        ...
        try {
            ...
            const { svc: svc1, infoMessages: info1, errorMessages: error1 } = makeSvc(targetHelper, multiBytes);
            await svc1.syncPublicKeys('all');
            ...
            const { svc: svc2, infoMessages: info2, errorMessages: error2 } = makeSvc(targetHelper, multiBytes);
            await svc2.syncPublicKeys('all');
    ...
    it('5. full bridge: syncPublicKeys exports from Windows agent and imports into Linux keyring', async function () {
        ...
        expect(fingerprint, 'TEST_KEY_FINGERPRINT must be set (check devcontainer.json remoteEnv and requestProxyRunTest.ts)').to.have.length.greaterThan(0);
        ...
        try {
            ...
            expect(hasFpr, `Windows test key ${fprNormalized} must be present in Linux keyring after sync`).to.be.true;
        } finally {
