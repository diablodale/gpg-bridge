...
describe('Phase 2 — request-proxy → agent-proxy → Real gpg-agent', function () {
  ...
  before(async function () {
    ...
    if (!keygrip) {
      throw new Error(
        'TEST_KEY_KEYGRIP is not set. Check runTest.ts extensionTestsEnv and devcontainer.json remoteEnv.',
      );
    }
  });
...
describe('Phase 5 — RequestProxy uses GpgCli for socket path', function () {
  ...
  let listdirs = (name: string) =>
    spawnSync('gpgconf', ['--list-dirs', name], {
      encoding: 'latin1',
      timeout: 5000,
      shell: false,
    });
  ...
  it('1. active socket path matches gpgconf --list-dirs agent-socket', function () {
    ...
    expect(proxySocketPath, 'proxy socket path must match gpgconf agent-socket').to.equal(
      expectedPath,
    );
  });
  ...
  it('2. stale files at agent-socket and agent-extra-socket are removed on start()', async function () {
    ...
    expect(fs.existsSync(extraPath), 'stale agent-extra-socket file must be removed on start()').to
      .be.false;
    ...
    try {
      ...
      expect(
        greeting,
        'proxy must accept connections at the gpgconf-resolved agent-socket path',
      ).to.match(/^OK/);
    } finally {
      client.close();
    }
...
// ---------------------------------------------------------------------------
// Phase 6 — syncPublicKeys end-to-end import (real gpg; stubbed executeCommand)
// ---------------------------------------------------------------------------
// Import from the compiled output so Node16 module resolution finds the .d.ts
// declaration alongside the .js file (generated by the main tsc build).
// GpgTestHelper extends GpgCli so it satisfies IGpgCliFactory.create(): GpgCli.
// eslint-disable-next-line @typescript-eslint/no-require-imports
const { PublicKeySync } =
  require('../../services/publicKeySync') as typeof import('../../out/services/publicKeySync');
...
describe('Phase 6 — PublicKeySync with isolated gpg', function () {
  ...
  function makeSvc(
    gpg: GpgTestHelper,
    keyBytes: string,
  ): { svc: typeof PublicKeySync.prototype; infoMessages: string[]; errorMessages: string[] } {
  ...
  // -----------------------------------------------------------------------
  // 2. Re-import same key: unchanged: 1, imported: 0
  // -----------------------------------------------------------------------
  ...
  it('3. info message contains the GPG Bridge prefix and correct count summary', async function () {
    ...
    expect(msg, 'message should start with GPG Bridge prefix').to.match(
      /^Public key sync complete:/i,
    );
  ...
  // -----------------------------------------------------------------------
  // 4. Import multiple groups of keys with duplicates
  // -----------------------------------------------------------------------
  it('4. Import multiple groups of keys with duplicates', async function () {
    ...
    try {
      ...
      multiSourceHelper = new GpgTestHelper();
      await multiSourceHelper.generateKey('Phase6 Sync User 1', 'phase6sync1@example.com');
      await multiSourceHelper.generateKey('Phase6 Sync User 2', 'phase6sync2@example.com');
      multiBytes = await multiSourceHelper.exportPublicKeys();
      expect(multiBytes.length, 'exported key bytes must be non-empty').to.be.greaterThan(0);

      // import into target — should report imported: 2, unchanged: 0
      const {
        svc: svc1,
        infoMessages: info1,
      ...
      // import again into target — should report imported: 1, unchanged: 2
      const {
        svc: svc2,
        infoMessages: info2,
  ...
  // -----------------------------------------------------------------------
  // 5. Full bridge end-to-end: VS Code command routes export through
  //    gpg-bridge-agent (Windows) and imports into the Linux keyring
  //
  // The real vscode.commands.executeCommand is injected as executeCommand so
  // the cross-host RPC to _gpg-bridge-agent.exportPublicKeys is real.
  // gpgCli is the default GpgCli (no factory override) so gpg --import runs
  // against the container's default keyring.
  // Messages are captured so the test can assert success vs. error.
  //
  // TEST_KEY_FINGERPRINT is forwarded from Windows via devcontainer.json
  // remoteEnv ${localEnv:TEST_KEY_FINGERPRINT}.
  // -----------------------------------------------------------------------
  it('5. full bridge: syncPublicKeys exports from Windows agent and imports into Linux keyring', async function () {
    const fingerprint = process.env.TEST_KEY_FINGERPRINT ?? '';
    expect(
      fingerprint,
      'TEST_KEY_FINGERPRINT must be set (check devcontainer.json remoteEnv and requestProxyRunTest.ts)',
    ).to.have.length.greaterThan(0);

    const fprNormalized = fingerprint.replace(/[^a-fA-F0-9]/g, '').toUpperCase();
    const infoMessages: string[] = [];
    const errorMessages: string[] = [];

    // Directly instantiate PublicKeySync with the real VS Code cross-host executeCommand
    // and captured message callbacks for observability. No gpgCliFactory override —
    // GpgCli defaults to the container's default keyring (no GNUPGHOME override).
    const svc = new PublicKeySync(
      { logCallback: undefined },
      {
    ...
    try {
      // 'pairs' exports only keys with a matching local secret key on Windows.
      ...
      expect(listResult.status, `gpg --list-keys failed: ${listResult.stderr}`).to.equal(0);

      const hasFpr = listResult.stdout
        .split('\n')
        .filter((line) => line.startsWith('fpr:'))
        .some((line) => line.split(':')[9]?.toUpperCase() === fprNormalized);
      expect(
        hasFpr,
