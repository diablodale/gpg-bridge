...
class RequestSessionManager extends EventEmitter implements ISessionManager {
  ...
  public handleIncomingData(chunk: Buffer): void {
    ...
    if (!validStates.includes(this.state)) {
      this.emit(
        'ERROR_OCCURRED',
        `Protocol violation: client sent ${chunk.length} bytes in state ${this.state}`,
      );
      return;
    }
  ...
  private handleAgentResponseComplete(response: string): void {
    ...
    if (completion.type === 'INQUIRE') {
      ...
      log(
        this.config,
        `[${this.sessionId}] Warning: Unexpected response format: ${sanitizeForLog(response)}`,
      );
  ...
  private async handleCleanupRequested(hadError: boolean): Promise<void> {
    ...
    try {
      ...
      log(
        this.config,
        `[${this.sessionId}] Error: failed to remove session event listeners during cleanup`,
      );
    }
  }
...
export class RequestProxy {
  ...
  async start(): Promise<void> {
    ...
    if (!agentSocketPath) {
      throw new Error(
        'Could not determine local GPG socket path. Is gpg installed? Try: gpgconf --list-dirs',
      );
    }
