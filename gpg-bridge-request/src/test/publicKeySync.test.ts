...
class MockGpgCliWithImport extends MockGpgCli {
  ...
  override async importPublicKeys(
    keyData: string,
  ): Promise<{ imported: number; unchanged: number; errors: number }> {
    if (this.importShouldThrow) {
      throw this.importShouldThrow;
    }
...
function createMockState(): MockState {
  ...
  const executeCommand = (command: string, ...args: unknown[]): Promise<unknown> => {
    ...
    if (exportThrows) {
      return Promise.reject(exportThrows);
    }
...
describe('PublicKeySync', () => {
  ...
  it("4. syncPublicKeys(['uid with spaces', 'FP2']) forwards array verbatim to executeCommand", async () => {
    ...
    expect(m.executeCommandCalls[0].args).to.deep.equal([
  ...
  it("8. autoSync('') — no-op; executeCommand never called", async () => {
    ...
    expect(
      m.executeCommandCalls,
      'executeCommand must not be called for empty setting',
    ).to.have.length(0);
  ...
  it('8b. autoSync([]) — no-op; executeCommand never called for empty array', async () => {
    ...
    expect(
      m.executeCommandCalls,
      'executeCommand must not be called for empty array',
    ).to.have.length(0);
    expect(m.gpgCli.importCalls).to.have.length(0);
  });
  ...
  it('8d. autoSync(arbitrary string) — shows error and does not call executeCommand', async () => {
    ...
    expect(
      m.executeCommandCalls,
      'executeCommand must not be called for invalid string',
    ).to.have.length(0);
  ...
  it('9. autoSync fires exactly once per call — no internal state causes re-triggering', async () => {
    ...
    expect(
      m.executeCommandCalls,
      'second call: exactly two total executeCommand calls',
    ).to.have.length(2);
