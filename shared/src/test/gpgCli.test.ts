...
function execFails(exitCode: number, stdout = '', stderr = ''): ExecFileFn {
  return () =>
    Promise.reject(
      Object.assign(new Error('process exited with non-zero'), { code: exitCode, stdout, stderr }),
    );
}
...
function execCapture(
  stdout: string,
  stderr = '',
): { fn: ExecFileFn; lastArgs: { binary: string; args: readonly string[]; opts: object } | null } {
  const capture = {
    fn: null as unknown as ExecFileFn,
    lastArgs: null as { binary: string; args: readonly string[]; opts: object } | null,
  };
...
function spawnCapture(stderr: string): {
  fn: SpawnForStdinFn;
  lastInput: Buffer | null;
  lastArgs: readonly string[] | null;
} {
  const capture = {
    fn: null as unknown as SpawnForStdinFn,
    lastInput: null as Buffer | null,
    lastArgs: null as readonly string[] | null,
  };
...
const FAKE_GPGCONF_EXE = path.join(
  FAKE_BIN,
  process.platform === 'win32' ? 'gpgconf.exe' : 'gpgconf',
);
const FAKE_GPG_EXE = path.join(FAKE_BIN, process.platform === 'win32' ? 'gpg.exe' : 'gpg');
const FAKE_GPGCONF_BIN = path.join(
  FAKE_BIN,
  process.platform === 'win32' ? 'gpgconf.exe' : 'gpgconf',
);
...
describe('GpgCli', () => {
  describe('constructor', () => {
    it('throws when gpgBinDir points to a directory without gpgconf[.exe]', () => {
      expect(
        () =>
          new GpgCli({ gpgBinDir: FAKE_BIN }, { existsSync: () => false, whichSync: whichMiss }),
    ...
    it('succeeds via PATH probe when whichSync returns a path', () => {
      ...
      expect(
        () => new GpgCli({}, { existsSync: () => false, whichSync: whichReturns(whichPath) }),
      ).not.to.throw();
    });
  ...
  describe('getBinDir()', () => {
    ...
    it('returns dirname of the path found by whichSync when gpgBinDir is omitted', () => {
      const whichPath = '/usr/bin/gpgconf';
      const cli = new GpgCli({}, { existsSync: () => false, whichSync: whichReturns(whichPath) });
  ...
  describe('listPublicKeys()', () => {
    ...
    it('passes correct arguments to execFileAsync', async () => {
      ...
      expect(capture.lastArgs!.binary).to.equal(FAKE_GPG_EXE);
      expect(capture.lastArgs!.args).to.deep.equal([
        '--list-keys',
        '--with-colons',
        '--with-secret',
      ]);
...
describe('parsePairedKeys()', () => {
  ...
  it('parses complex output: revoked key, multiple subkeys, grp: records, multiple UIDs', () => {
    ...
    expect(result[0].userIds).to.deep.equal(['Test User One (primary) <one@example.com>']);
...
describe('unescapeGpgColonField()', () => {
  it('returns plain strings unchanged', () => {
    expect(unescapeGpgColonField('Alice <alice@example.com>')).to.equal(
      'Alice <alice@example.com>',
    );
  });
