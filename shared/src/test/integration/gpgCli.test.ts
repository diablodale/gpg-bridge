...
describe('GpgCli integration (via GpgTestHelper)', function () {
  ...
  describe('GpgTestHelper constructor', () => {
    it('sets gnupgHome to a non-empty string pointing to a real temp directory', async () => {
      ...
      try {
        ...
        expect(
          fs.existsSync(helper.gnupgHome),
          `expected dir to exist: ${helper.gnupgHome}`,
        ).to.equal(true);
      } finally {
    ...
    it('does NOT mutate process.env.GNUPGHOME', async () => {
      ...
      try {
        expect(after).to.equal(
          before,
          'GpgTestHelper constructor must not mutate process.env.GNUPGHOME',
        );
  ...
  describe('GpgTestHelper inherited methods', () => {
    ...
    it('listPairedKeys() returns correct PairedKeyInfo[] for a generated key', async () => {
      ...
      const key = keys[0];
      expect(key.fingerprint).to.have.lengthOf(
        40,
        `expected 40-char fingerprint, got: ${key.fingerprint}`,
      );
      expect(
        /^[0-9A-F]{40}$/i.test(key.fingerprint),
        `fingerprint should be hex: ${key.fingerprint}`,
      ).to.equal(true);
    ...
    it('exportPublicKeys() returns non-empty armored string for a key pair', async () => {
      ...
      expect(result.length).to.be.greaterThanOrEqual(
        300,
        'expected at least 300 chars for an Ed25519+cv25519 key pair export',
      );
    });
    ...
    it('importPublicKeys() imports into a second keyring; imported: 1', async () => {
      ...
      expect(keyData.length).to.be.greaterThanOrEqual(
        300,
        'expected at least 300 chars for an Ed25519+cv25519 key pair export',
      );
