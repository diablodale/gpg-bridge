  ...
  connect(socketPath: string): Promise<string> {
    return new Promise((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        socket.destroy();
        reject(new Error(`AssuanSocketClient: timed out waiting for greeting on ${socketPath}`));
      }, this.timeoutMs);
    ...
    return new Promise((resolve, reject) => {
      ...
      const resetTimer = () => {
        ...
        timerHandle = setTimeout(() => {
          socket.removeListener('data', onData);
          socket.removeListener('error', onError);
          reject(new Error(`AssuanSocketClient: timed out waiting for response to: ${cmd.trim()}`));
        }, this.timeoutMs);
      };
      ...
      const onData = (chunk: string) => {
        ...
        if (completion.type === 'INQUIRE') {
          ...
          const inquireLine = buffer.split('\n').find((l) => l.startsWith('INQUIRE ')) ?? '';
          ...
          socket.removeListener('data', onData);
          socket.removeListener('error', onError);
          reject(new Error(`AssuanSocketClient: unexpected INQUIRE: ${inquireLine}`));
          return;
        }
