...
describe('Protocol Utilities', () => {
    ...
    describe('Response Completion Detection', () => {
        ...
        describe('ERR responses', () => {
            ...
            it('detects ERR with detailed message', () => {
                const result = detectResponseCompletion('ERR 100663404 Inappropriate ioctl for device <Pinentry>\n');
                assert.strictEqual(result.complete, true);
                assert.strictEqual(result.type, 'ERR');
            });
        });
        ...
        describe('Edge cases', () => {
            ...
            it('handles realistic KEYINFO response', () => {
                const response = 'S KEYINFO D27BB288411333745EE1B194FBC6162A92775BA4 T E77DE0715C50F8253FA55E7F78A8ACB2D65CC0FA - - - P - - -\nOK\n';
                const result = detectResponseCompletion(response);
                assert.strictEqual(result.complete, true);
                assert.strictEqual(result.type, 'OK');
            });
    ...
    describe('Socket Cleanup', () => {
        ...
        function createMockSocket(opts: { throwOnRemove?: boolean; throwOnDestroy?: boolean } = {}): MockSocket {
            return {
        ...
        it('should return error when removeAllListeners throws', () => {
            ...
            assert.strictEqual(mockSocket.removeCalled, true, 'Should call removeAllListeners despite error');
            assert.strictEqual(mockSocket.destroyCalled, true, 'Should still call destroy after removeAllListeners error');
        });
