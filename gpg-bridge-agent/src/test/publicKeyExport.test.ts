...
describe('exportPublicKeys', () => {
    ...
    it('filter=undefined: QuickPick is shown, populated with items from listPublicKeys()', async () => {
        ...
        expect(gpgCli.listPairedKeysCalled, 'listPairedKeys should not be called; hasSecret comes from --with-secret').to.be.false;
    ...
    it('QuickPick grouping: normal keys first sorted by UID, separator, then revoked/expired sorted by UID', async () => {
        gpgCli.listPublicKeysResult = [
            { fingerprint: 'R'.padEnd(40, '0'), userIds: ['Zelda <z@example.com>'],   revoked: true,  expired: false },
            { fingerprint: 'B'.padEnd(40, '0'), userIds: ['Charlie <c@example.com>'], revoked: false, expired: false },
            { fingerprint: 'E'.padEnd(40, '0'), userIds: ['Aaron <a@example.com>'],   revoked: false, expired: true  },
            { fingerprint: 'N'.padEnd(40, '0'), userIds: ['Bob <b@example.com>'],     revoked: false, expired: false },
        ];
        const { quickPick, calls } = makeQuickPickDep([]);
