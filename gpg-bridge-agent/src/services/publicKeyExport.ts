...
export async function exportPublicKeys(
  ...
  const quickPick =
    deps.quickPick ??
    ((items, options) =>
      vscode.window.showQuickPick(items, options) as Promise<
  ...
  if (keyData.length === 0) {
    showWarningMessage(
      'No public key data exported. The filter did not match any keys in the keyring.',
    );
    return undefined;
  }
