...
export class AgentSessionManager extends EventEmitter implements ISessionManager {
  ...
  private handleClientConnectRequested(payload: EventPayloads['CLIENT_CONNECT_REQUESTED']): void {
    ...
    this.connectionTimeout = setTimeout(() => {
      log(
        this.config,
        `[${this.sessionId}] Connection timeout after ${this.config.connectionTimeoutMs}ms`,
      );
  ...
  private handleAgentWriteOk(payload: EventPayloads['AGENT_WRITE_OK']): void {
    ...
    if (requiresTimeout) {
      this.agentDataTimeout = setTimeout(() => {
        log(
          this.config,
          `[${this.sessionId}] Greeting timeout after ${this.config.greetingTimeoutMs}ms`,
        );
        this.emit('ERROR_OCCURRED', {
  ...
  private handleClientDataReceived(payload: EventPayloads['CLIENT_DATA_RECEIVED']): void {
    ...
    const logMsg = isNonce ? `${commandBlock.length}-byte nonce` : sanitizeForLog(commandBlock);
  ...
  private handleAgentDataChunk(payload: EventPayloads['AGENT_DATA_CHUNK']): void {
    ...
    if (completion.complete) {
      log(
        this.config,
        `[${this.sessionId}] Complete response (${completion.type}): ${sanitizeForLog(this.buffer)}`,
      );
...
export class AgentProxy {
  ...
  public async sendCommands(
    ...
    if (session.getState() !== 'READY') {
      const error = new Error(
        `Protocol violation: sendCommands called while session in ${session.getState()}`,
      );
    ...
    return new Promise<{ response: string }>((resolve, reject) => {
      ...
      const handleCleanup = () => {
        ...
        // Use stored error if available, otherwise generic message
        const error =
          session.getLastError() ?? new Error('Session closed while waiting for command response');
        reject(error);
      };
